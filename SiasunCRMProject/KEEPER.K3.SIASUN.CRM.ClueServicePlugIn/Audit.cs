    ...
    public class Audit: AbstractOperationServicePlugIn
    {
        ...
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            if (e.DataEntitys!=null&&e.DataEntitys.Count()>0)
            {
                ...
                {
                    CRMCustIds = new List<long>();//线索转化成CRM客户的源单ID集合
                    CRMLinkIds = new List<long>();//线索转化成CRM联系人的源单ID集合
                    CRMOppIds = new List<long>();//线索转化成商业机会的源单ID集合
                    //线索类型是是转商机客户
        ...
        /// 创建CRM客户
        /// </summary>
        /// <param name="CRMCustIds">线索ID</param>
        private void CreateCRMCust(List<long> CRMCustIds)
        {
            //下推CRM客户CRM_CUST
            ConvertOperationResult operationCustResult = CRMServiceHelper.ConvertBills(base.Context, this.BusinessInfo.GetForm().Id, "CRM_CUST", CRMCustIds);
            //获取下推生成的下游单据数据包
